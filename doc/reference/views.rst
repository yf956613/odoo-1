...
``field``
    ...
    ``sum``, ``avg``
        ...
        ``group_operator``
    ``attrs``
        dynamic attributes based on record values. Only affects the current
...
The fields can be displayed differently by specifying a ``widget=" "`` attribute.

A wide variety of widgets are available in Odoo by default.
For detailed information on Odoo widgets, please refer to the :ref:`Javascript Widget Reference <reference/js/widgets>`.
