...
class TestSaleSignature(odoo.tests.HttpCase):
    def test_01_portal_sale_signature_tour(self):
        ...
        portal_user = self.env['res.users'].create({'name': 'Demo Portal Test', 'company_id': self.env.ref("base.main_company").id, 'login': 'portalTest', 'password': 'portalTest', 'groups_id': [(4, self.env.ref('base.group_portal').id)]})
