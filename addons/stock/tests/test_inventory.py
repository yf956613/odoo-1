...
class TestInventory(TransactionCase):
    ...
    def test_inventory_8(self):
        """ Checks that inventory adjustment lines values are updated after some
        operations added quantity for a inventoried product.
        """
        ...
        inventory.action_start()
        inventory.line_ids.product_qty = 9
        self.assertEqual(inventory.line_ids.theoretical_qty, 7)
        self.assertEqual(inventory.line_ids.product_qty, 9)

        # Creates new quant with 5 additional product and expects theoretical
        # quantity was updated to 12 and counted quantity was recomputed to 14.
        vals = {
            'product_id': self.product1.id,
            'product_uom_id': self.uom_unit.id,
            'location_id': self.stock_location.id,
        ...
        self.env['stock.quant'].create(vals)
        self.assertEqual(inventory.line_ids.theoretical_qty, 12)
        self.assertEqual(inventory.line_ids.product_qty, 14)

    def test_inventory_9(self):
        """ Checks that inventory adjustment lines values are updated after a
        quant is manually updated.
        """
        # Set initial quantity to 7
        vals = {
            'product_id': self.product1.id,
        ...
        # Decreases quant and expects theoretical quantity was updated to 3 and
        # counted quantity was recomputed to 5.
        quant.update({'inventory_quantity': 3})
        self.assertEqual(inventory.line_ids.theoretical_qty, 3)
        self.assertEqual(inventory.line_ids.product_qty, 5)

    def test_inventory_10(self):
        """ Checks that inventory adjustment lines values are updated when an
        another inventory adjustment with common product is validated.
        """
        # Set initial quantity to 7
        vals = {
            'product_id': self.product1.id,
        ...
        self.assertEqual(inventory_2.line_ids.theoretical_qty, 7)

        # Set product quantity to 10 in inventory 1 and to 8 in inventory 2,
        # then validate inventory 2
        inventory_1.line_ids.product_qty = 10
        inventory_2.line_ids.product_qty = 8
        inventory_2.action_validate()
        # Expects product and theoretical quantities of inventory 1 were been
        # increased by 1 (according to difference of inventory 2) but kept its
        # own difference scale
        self.assertEqual(inventory_1.line_ids.theoretical_qty, 8)
        self.assertEqual(inventory_1.line_ids.difference_qty, 3)
        self.assertEqual(inventory_1.line_ids.product_qty, 11)
