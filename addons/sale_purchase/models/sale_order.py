...
class SaleOrderLine(models.Model):
    ...
    def write(self, values):
        ...
        if 'product_uom_qty' in values:
            increased_lines = self.sudo().filtered(lambda r: r.purchase_line_count and float_compare(r.product_uom_qty, values['product_uom_qty'], precision_digits=r.product_uom.decimal_places) == -1)
            decreased_lines = self.sudo().filtered(lambda r: r.purchase_line_count and float_compare(r.product_uom_qty, values['product_uom_qty'], precision_digits=r.product_uom.decimal_places) == 1)
