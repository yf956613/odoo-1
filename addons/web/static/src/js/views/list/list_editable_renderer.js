...
ListRenderer.include({
    ...
    /**
     * Adds the appropriate properties to the list according to its editable state.
     * Classes are toggled to indicate whether the list is empty and/or editable, resize
     * handles are added if necessary and columns widths are then frozen if needed.
     */
    _applyEditableProperties: function () {
        const table = this.el.getElementsByTagName('table')[0];
        if (table) {
            table.classList.toggle('o_empty_list', !this._hasVisibleRecords(this.state));
            table.classList.toggle('o_editable_list', this.isEditable());
            if (this.isEditable()) {
                this._freezeColumnWidths();
            }
        }
    ...
    _renderView: function () {
        this.currentRow = null;
        return this._super.apply(this, arguments).then(() => this._applyEditableProperties());
    },
    /**
