...
class ResCompany(models.Model):
    ...
    def _autorise_lock_date_changes(self, vals):
        ...
        * You cannot delete a tax lock date, lock a period that is not finished yet or the tax lock date must be set after
        ...
        tax_lock_date = vals.get('tax_lock_date') and\
            fields.Date.from_string(vals['tax_lock_date'])
        ...
        for company in self:
            ...
            if old_tax_lock_date and not tax_lock_date and 'tax_lock_date' in vals:
                raise ValidationError(_('The tax lock date is irreversible and can\'t be removed.'))
            ...
            if old_tax_lock_date and tax_lock_date and tax_lock_date < old_tax_lock_date:
                raise ValidationError(_('The new tax lock date must be set after the previous lock date.'))

            # In case of no new tax lock date in vals, fallback to the oldest
            if not tax_lock_date:
                if old_tax_lock_date:
                    tax_lock_date = old_tax_lock_date
            ...
            if tax_lock_date and tax_lock_date > previous_month:
                raise ValidationError(_('You cannot lock a period that is not finished yet. Please make sure that the tax lock date is not set after the last day of the previous month.'))
