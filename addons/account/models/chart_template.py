...
class AccountChartTemplate(models.Model):
    ...
    def load_for_current_company(self, sale_tax_rate, purchase_tax_rate):
        ...
        if acc_template_ref.get(self.default_pos_receivable_account_id.id):
            company.write({
                'default_pos_receivable_account_id': acc_template_ref[self.default_pos_receivable_account_id.id]
            })
        else:
            property_receivable = self.env['account.account'].browse(acc_template_ref[self.property_account_receivable_id.id])
            pos_receivable = property_receivable.copy({'name': '%s (PoS)' % property_receivable.name})
            company.write({
                'default_pos_receivable_account_id': pos_receivable.id
            })
