odoo.define('mail.websitelivechatwindow_test', function (require) {
"use strict";
...
QUnit.test('define several livechat colors', async function (assert) {
    ...
    testUtils.mock.addMockEnvironment(parent, {debug:true});
    await chatWindow.appendTo($('#qunit-fixture'));
    chatWindow.render();
    assert.strictEqual(chatWindow.$('.o_thread_window_header').css('background-color'), 'rgb(255, 255, 0)',
        "header background color should be yellow");
    assert.strictEqual(chatWindow.$('.o_thread_window_header').css('color'), 'rgb(0, 128, 0)',
