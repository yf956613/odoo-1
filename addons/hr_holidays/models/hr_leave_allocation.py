...
class HolidaysAllocation(models.Model):
    ...
    second_approver_id = fields.Many2one(
        ...
        help='This area is automaticly filled by the user who validates the allocation with second level (If time off type need second validation)')
    ...
    def _check_approval_update(self, state):
        ...
        for allocation in self:
            val_type = allocation.validation_type
            ...
            if not is_manager and state != 'confirm':
                if state == 'draft':
                    ...
                    if val_type == 'no_validation' and current_employee == allocation.employee_id:
                        continue
                    ...
                    if (state == 'validate1' and val_type == 'both') or (state == 'validate' and val_type == 'manager') and allocation.holiday_type == 'employee':
                        if not is_officer and self.env.user != allocation.employee_id.leave_manager_id:
                            raise UserError(_('You must be either %s\'s manager or Time Off manager to approve this allocation') % (allocation.employee_id.name))

                    if (state == 'validate' and val_type == 'both') and not is_officer:
                        raise UserError(_("Only a Time Off manager can do the second approval."))
                    if (state == 'validate1' and val_type == 'both') and is_officer and self.env.user != allocation.employee_id.leave_manager_id:
                        raise UserError(_("A Time Off manager can only do the second approval. %s\'s manager should first approve the allocation"))
    # ------------------------------------------------------------
    # Activity methods
    # ------------------------------------------------------------
