...
class LunchOrder(models.Model):
    ...
    # TODO Try to remove this, at least make it a compute dependant on the context
    ...
    def create(self, values):
        ...
        if lines:
            # FIXME This will update multiple lines in the case there are multiple matching lines
            # which should not happen trough the interface
    ...
    def write(self, values):
        ...
        # Only write on topping_ids_1 because they all share the same table and we don't want to remove all the records
        # _extract_toppings will pop topping_ids_1, topping_ids_2 and topping_ids_3 from values
        # This also forces us to invalidate the cache for topping_ids_2 and topping_ids_3 that could have changed
        # through topping_ids_1 without the cache knowing about it
        toppings = self._extract_toppings(values)
        ...
        if merge_needed:
            ...
            for line in self:
                ...
                if matching_lines:
                    ...
                    # TODO Try to batch it, be careful there might be multiple matching lines
                    # for the same order hence quantity should not always be line.quantity, but rather a sum
