...
class OgoneController(http.Controller):
    ...
    def plop(self):
        ...
        # tx = token.validate(**params)  # calls _ogone_form_validate ?
        # print(tx.read())
        # res = {'result': True, 'id': token.id, 'short_name': token.short_name, '3d_secure': True,
        #        'verified': token.verified}
        # if tx and tx.html_3ds:
        #     res['3d_secure'] = tx.html_3ds
        # print("tx = ", tx.read())

        # From /website_payment/transaction/<string:reference>/<string:amount>/<string:currency_id> in porta.py
        order_id, reference, amount, currency_id,  = None # nécessaire et manquant
        ...
        values = {
            ...
            'partner_id': int(post.get('partner_id')),
            'type': 'form_save' if acquirer.save_token != 'none' and int(post.get('partner_id')) else 'form',
        }
        if order_id:
            values['sale_order_ids'] = [(6, 0, [order_id])]
        # PaymentProcessing.add_payment_transaction(tx)
        tx_id = PaymentProcessing.get_payment_transaction_ids()
        a = request.env['payment.transaction'].browse(tx_id)
        return werkzeug.utils.redirect(f"/website_payment/token/{reference}/{amount}/{currency_id}")
        # return werkzeug.utils.redirect('/payment/ogone/s2s/create_json_3ds')
