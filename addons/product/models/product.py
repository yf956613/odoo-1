...
class ProductProduct(models.Model):
    ...
    def _compute_variant_item_count(self):
        for product in self:
            template_domain = expression.AND([[('product_tmpl_id', '=', product.product_tmpl_id.id)], [('applied_on', '=', '1_product')]])
            variant_domain = expression.AND([[('product_id', '=', product.id)], [('applied_on', '=', '0_product_variant')]])
            domain = expression.OR([variant_domain, template_domain])
    ...
    def open_pricelist_rules(self):
        self.ensure_one()
        template_domain = expression.AND([[('product_tmpl_id', '=', self.product_tmpl_id.id)], [('applied_on', '=', '1_product')]])
        variant_domain = expression.AND([[('product_id', '=', self.id)], [('applied_on', '=', '0_product_variant')]])
        domain = expression.OR([variant_domain, template_domain])
        return {
            'name': _('Price Rules'),
            'view_mode': 'tree,form',
