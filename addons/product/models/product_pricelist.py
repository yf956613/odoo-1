...
class PricelistItem(models.Model):
    ...
    def _default_pricelist_id(self):
        domain = expression.OR([[('company_id', '=', False)], [('company_id', '=', self.env.company.id)]])
        pricelist = self.env['product.pricelist'].search(domain, limit=1)
        if not pricelist and self.user_has_groups('product.group_product_pricelist') and not self.user_has_groups('product.group_sale_pricelist'):
            # If pricelists manipulation isn't enabled and no pricelist was found:
    ...
    def _write(self, values):
        ...
        return super(PricelistItem, self)._write(values)
