...
class report_product_pricelist(models.AbstractModel):
    ...
    def _get_report_data(self, active_model, product_ids, pricelist_id, quantities, report_type='html'):
        products = []
        ...
        if is_product_tmpl:
            records = self.env['product.template'].browse(product_ids)
            ...
            records = self.env['product.product'].browse(product_ids)
            for product in records:
