...
class PosConfig(models.Model):
    ...
    discount_product_id = fields.Many2one('product.product', string='Discount Product',
        domain="[('purchase_ok', '=', False), ('sale_ok', '=', False), ('taxes_id', '=', False)]", help='The product used to model the discount.')
    ...
    def _onchange_module_pos_discount(self):
        if self.module_pos_discount and not self.discount_product_id:
            self.discount_product_id = self.env['product.product'].search([('name', '=', 'Discount')], limit = 1)
            if not self.discount_product_id:
                self.discount_product_id = self.env['product.product'].create({
                    'sale_ok': False,
                    'purchase_ok': False,
                    'name': 'Discount',
                    'default_code': 'DISC',
