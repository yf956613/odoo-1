...
class PosOrder(models.Model):
    ...
    def _process_order(self, pos_order):
        ...
        for payments in pos_order['statement_ids']:
            if not float_is_zero(payments[2]['amount'], precision_digits=prec_acc):
                self.env['pos.payment'].create(self._payment_fields(order, payments[2]))
    ...
    is_fully_paid = fields.Boolean('Is Fully Paid', compute='_compute_is_fully_paid')
    amount_to_pay = fields.Float('To pay', currency_field='currency_id', compute='_compute_amount_to_pay')
    ...
    @api.depends('currency_id', 'amount_paid', 'amount_total')
    def _compute_is_fully_paid(self):
        for order in self:
            prec_acc = self.currency_id.decimal_places
            order.is_fully_paid = float_is_zero(order.amount_to_pay, precision_digits=prec_acc)

    @api.depends('amount_paid', 'amount_total')
    def _compute_amount_to_pay(self):
        for order in self:
            order.amount_to_pay = order.amount_total - order.amount_paid
