...
class TestPointOfSaleCommon(TransactionCase):
    ...
    def setUp(self):
        ...
        self.credit_payment_method = self.env['pos.payment.method'].create({
            'name': 'Credit',
            'receivable_account_id': self.company.default_pos_receivable_account_id.id,
...
class TestPoSCommon(TransactionCase):
    ...
    def setUp(self):
        ...
        self.pos_manager = self.env['res.users'].browse(2)
        self.env = self.env(user=self.pos_manager)

        # Set basic defaults
        self.company = self.env.company
        self.pos_sale_journal = self.env.ref('point_of_sale.pos_sale_journal')
        self.invoice_journal = self.env['account.journal'].search([
            ('company_id', '=', self.company.id),
            ('type', '=', 'sale'),
            ('id', '!=', self.pos_sale_journal.id),
        ], limit=1)
        ...
        self.pos_receivable_account = self.company.default_pos_receivable_account_id
    ...
    def _create_basic_config(self):
        ...
        credit_payment_method = self.env['pos.payment.method'].create({
            'name': 'Credit',
            'receivable_account_id': self.pos_receivable_account.id,
            'split_transactions': True,
        })
        with Form(main_config) as config:
            config.module_account = True
            config.invoice_journal_id = self.invoice_journal
            config.journal_id = self.pos_sale_journal
            config.payment_method_ids.add(credit_payment_method)
    ...
    def _create_other_currency_config(self):
        ...
        other_currency_config = self.env.ref('point_of_sale.pos_config_main').copy()
        with Form(other_currency_config) as config:
            config.name = 'Shop Other'
            config.invoice_journal_id = other_invoice_journal
            config.journal_id = other_sales_journal
            config.use_pricelist = True
            config.available_pricelist_ids.clear()
            config.available_pricelist_ids.add(other_pricelist)
            config.pricelist_id = other_pricelist
            config.payment_method_ids.clear()
            config.payment_method_ids.add(other_cash_payment_method)
            config.payment_method_ids.add(other_bank_payment_method)

        return other_currency_config
    ...
    def create_ui_order_data(self, product_quantity_pairs, customer=False, is_invoiced=False, payments=None, uid=None, **kwargs):
        """ Replicate the order_data generated by the pos ui.
        ...
        if payments is None:
            statement_ids = [create_payment(self.cash_pm, total_amount_incl)]
        else:
            statement_ids = [
        ...
        return {
            'data': {
                'amount_paid': sum(payment[2]['amount'] for payment in statement_ids),
                'amount_return': 0,
    ...
    def open_new_session(self):
        ...
        Fields:
            ...
            * credit_pm : credit payment method of the session
        """
        self.config.open_session_cb()
        self.session = self.config.current_session_id
        self.currency = self.session.currency_id
        self.pricelist = self.session.config_id.pricelist_id
        self.cash_pm = self.session.payment_method_ids.filtered(lambda pm: 'Cash' in pm.name)
        self.bank_pm = self.session.payment_method_ids.filtered(lambda pm: 'Bank' in pm.name)
        self.credit_pm = self.session.payment_method_ids.filtered(lambda pm: 'Credit' in pm.name)
