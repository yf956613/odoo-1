...
class WebsiteVisitorTests(HttpCase):
    def test_create_visitor_on_tracked_page(self):
        ...
        self.assertEqual(len(Track.search([])), 0, "No track created after visiting an untracked page")
        ...
        self.assertEqual(len(Track.search([])), 1, "A track should be created after visiting a tracked page")
        self.url_open(tracked_page.url)
        self.assertEqual(len(Visitor.search([])), 1, "No visitor should be created after visiting another tracked page")
        self.assertEqual(len(Track.search([])), 1, "No track should be created after visiting another tracked page before 30 min")
        Visitor.search([]).unlink()
        Track.search([]).unlink
